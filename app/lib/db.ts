...
export function insertPendingOrder(input: PendingOrderInput): void {
  insertStmt.run({ ...input, podStatus: input.paymentMethod === "pod" ? "pending" : null });
}
...
// "delivered" means the rider collected the money, so it also marks the order
// paid. Any other status clears paid, so a mistaken "delivered" can be undone.
const setPodStatusStmt = db.prepare(`
  UPDATE orders
...
export function setPodStatus(reference: string, status: PodStatus): boolean {
  return setPodStatusStmt.run({ reference, status }).changes > 0;
}
